...
// ============================================================
// 首頁榮譽榜：使用 siteContent/homeHonors，避免額外 collection 索引。
// ============================================================
const honorsRef = doc(db, "siteContent", "homeHonors");
const honorListElement = document.getElementById("honor-list");
let honorItems = [];
let honorsDocumentExists = false;
let honorsSeedInProgress = false;
let editingHonorId = null;

const honorDialog = createEditorDialog({
    id: "honor-editor-dialog",
    title: "新增／修改榮譽",
    fields: [
        { name: "date", label: "日期", type: "date", required: true },
        {
            ...
            required: true,
            maxLength: 600,
            placeholder: "例如：榮獲 2026 全國智慧控制競賽第一名"
        }
    ]
});
...
function normalizeHonorItems(value) {
    if (!Array.isArray(value)) return [];

    return value
        .map((item, index) => ({
            id: String(item?.id ?? `honor-${index + 1}`),
            date: String(item?.date ?? "").trim(),
            content: String(item?.content ?? "").trim()
        }))
        .filter((item) => /^\d{4}\/\d{2}\/\d{2}$/.test(item.date) && item.content)
        .sort((a, b) => b.date.localeCompare(a.date));
}

function createHonorActions(item) {
    ...
    editButton.addEventListener("click", () => {
        try {
            ...
            fillDialog(honorDialog, {
                date: slashDateToInput(item.date),
    ...
    deleteButton.addEventListener("click", async () => {
        try {
            ...
            const nextItems = honorItems.filter((honor) => honor.id !== item.id);
            await saveHonorItems(nextItems);
            showToast("榮譽資料已刪除。", "success");
        } catch (error) {
            console.error("榮譽資料刪除失敗：", error);
...
function renderHonorItems() {
    honorListElement.replaceChildren();

    if (!honorItems.length) {
        const empty = document.createElement("p");
        empty.className = "honor-empty";
        empty.textContent = "目前尚未建立榮譽資料。";
        honorListElement.appendChild(empty);
        return;
    }
    ...
    honorItems.forEach((item) => {
        ...
        article.className = "honor-card admin-editable-item";
        ...
        article.append(date, content, createHonorActions(item));
        fragment.appendChild(article);
    });

    honorListElement.appendChild(fragment);
}

async function loadHonorItems() {
    try {
        const snapshot = await getDoc(honorsRef);
        honorsDocumentExists = snapshot.exists();
        honorItems = honorsDocumentExists
            ? normalizeHonorItems(snapshot.data().items)
            : [];
    } catch (error) {
        console.error("榮譽榜載入失敗：", error);
        honorItems = [];
    }
    renderHonorItems();
}

async function ensureHonorsDocument() {
    if (honorsDocumentExists || honorsSeedInProgress) return;

    honorsSeedInProgress = true;
    try {
        await setDoc(honorsRef, {
            items: honorItems,
            updatedAt: serverTimestamp()
        });
        honorsDocumentExists = true;
        showToast("已建立 siteContent/homeHonors。", "success", 3200);
    } catch (error) {
        console.error("建立榮譽榜文件失敗：", error);
        showToast("無法建立榮譽榜文件，請確認 Firestore Rules。", "error", 3800);
    } finally {
        honorsSeedInProgress = false;
    }
}

async function saveHonorItems(nextItems) {
    const normalizedItems = normalizeHonorItems(nextItems);
    await setDoc(honorsRef, {
        items: normalizedItems,
        updatedAt: serverTimestamp()
    });
    honorItems = normalizedItems;
    honorsDocumentExists = true;
    renderHonorItems();
}
...
honorDialog.editorForm.addEventListener("submit", async (event) => {
    event.preventDefault();

    try {
        requireAdminMode();
        setDialogBusy(honorDialog, true, "儲存中…");
        const values = Object.fromEntries(new FormData(honorDialog.editorForm));
        const item = {
            id: editingHonorId || createStableId("honor"),
            date: inputDateToSlash(values.date),
            content: values.content.trim()
        };

        const nextItems = editingHonorId
            ? honorItems.map((existing) => existing.id === editingHonorId ? item : existing)
            : [...honorItems, item];

        await saveHonorItems(nextItems);
        honorDialog.close();
        showToast(editingHonorId ? "榮譽資料已修改。" : "榮譽資料已新增。", "success");
    } catch (error) {
        console.error("榮譽資料儲存失敗：", error);
        setDialogError(honorDialog, error.message || "儲存失敗，請確認日期與管理員權限。");
    } finally {
        honorDialog.saveButton.disabled = false;
    }
});

const honorsLoadPromise = loadHonorItems();

onAdminStateChange(async (state) => {
    if (state.isAdmin) {
        await honorsLoadPromise;
        await ensureHonorsDocument();
    }
});
...
await Promise.all([homeLoadPromise, honorsLoadPromise, resetAndLoadNews(), galleryLoadPromise]);
renderGallery();
window.pageTransitionReady?.("首頁資料載入完成");
